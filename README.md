...
両アプリのいずれかで仕様変更が起きた場合、動かなくなる恐れがあります。バージョンアップ等を経緯にしていきなり動かなくなった場合は仕様変更などが考えられますので、このページの最下部にある連絡先から連絡ください。
...
#### ？.エラーが出て動きません
A. 細かいサポートはしてませんので、各自ググるなどして対応してください。X（旧Twitter）のDM等にエラー内容について要点をまとめて連絡いただければ、お返事できる場合はお返事します（必ず返事することは保証できません）。
